...
12. **license.dat writes — sirf `src/utils.py::save_license_dat()` se:** ye choke-point utf-8 + owner-only perms enforce karta hai (audit Batch-2/D4). Koi naya code seedha `open(get_data_path('license.dat'), 'w')` na likhe.
13. **Core zip = WHITELIST only:** Windows CI step (`release.yml`) ab `build_update.py` jaisa whitelist use karta hai (Batch-1/F1). Blacklist/copytree approach kabhi wapas mat laana — wo internal docs aur nrega-server secrets ship kar deta tha. Pre-release leak-check: `venv/bin/python scripts/_verify_whitelist_dryrun.py` (CLEAN + must-have files dono chahiye).
