...
class DeleteApplicantTab(BaseAutomationTab):
    ...
    def run_automation_logic(self, inputs):
        ...
        try:
            ...
            for jc_idx, (jobcard, applicants) in enumerate(grouped.items()):
                ...
                self.app.log_message(self.log_display, "🌐 Opening Delete Applicant page...")
                ...
                # ── 2. Village (JS — fast) ──
                try:
                    self.app.log_message(self.log_display, "🏘️  Selecting village from jobcard code...")
                    ...
                    """, v_dd, v_code)
                    if found:
                        self.app.log_message(self.log_display, f"     ✅ Village code {v_code} selected.")
                    else:
                ...
                try:
                    self.app.log_message(self.log_display, "📋 Selecting Registration No (Jobcard)...")
                    ...
                    """, reg_dd, jobcard)
                    if found_reg:
                        self.app.log_message(self.log_display, f"     ✅ Jobcard {jobcard} selected.")
                    else:
                        self.app.log_message(self.log_display, f"     ❌ Jobcard {jobcard} not found in registration dropdown.")
                        for nu, ov in applicants:
                ...
                # ── 4. Process table ──
                self.app.log_message(self.log_display, "📊 Scanning applicant table...")
                try:
                    ...
                    for norm_name, ov, original_upper in normalized_applicants:
                        ...
                        if norm_name in portal_names:
                            try:
                                ...
                                self.app.log_message(self.log_display, f"     ✔ Filled: {actual}")
                    ...
                    if filled_ok:
                        self.app.log_message(self.log_display, "📤 Submitting deletion...")
                        ...
                        try:
                            ...
                            alert.accept()
                            alert_fired = True
                            self.app.log_message(self.log_display, f"     Alert: {alert_text}")
                        ...
                        self.app.log_message(self.log_display, "     Waiting for postback result...")
                        try:
                        ...
                        if msg_text:
                            self.app.log_message(self.log_display, f"     📝 Page message: {msg_text}")
                        ...
                        if "deleted" in msg_text.lower() and "has been marked" in msg_text.lower():
                            ...
                            self.app.log_message(self.log_display, "     ✅ No error message — assuming success.")
                            for nu, ov in filled_ok:
                                log_one(jobcard, ov[3], "Success", "Submitted successfully.")
                    else:
                        self.app.log_message(self.log_display, "⚠️  No applicants were filled.", "warning")

                except Exception as e:
                    self.app.log_message(self.log_display, f"❌ Table error: {e}", "error")
                    for nu, ov in applicants:
            ...
            sep = "=" * 50
            summary = f"\n{sep}\n✅ Applicant deletions complete: {app_done} results."
            if reg_done:
                summary += f"\n🗑️  Registrations deleted: {reg_done}"
            summary += f"\n{sep}"
            self.app.log_message(self.log_display, summary)
            ...
            msg = f"Applicant deletion finished.\n{result_seq[0]} results."
            if pending_reg_del:
    ...
    def _process_reg_deletion(self, driver, wait, jobcard, panchayat,
        ...
        try:
            # Navigate
            self.app.log_message(self.log_display, "🌐 Opening Registration Delete page...")
            ...
            try:
                self.app.log_message(self.log_display, "🏘️  Selecting village...")
                v_code = jobcard.split('/')[0].split('-')[-1]
                v_dd = wait.until(EC.presence_of_element_located(
                    (By.ID, "ctl00_ContentPlaceHolder1_ddlvillage")))
                found = driver.execute_script("""
                ...
                """, v_dd, v_code)
                if not found:
                    self.app.log_message(self.log_display, f"     ❌ Village code '{v_code}' not found.")
            ...
            try:
                self.app.log_message(self.log_display, f"📋 Selecting Registration: {jobcard}")
                reg_dd = wait.until(EC.presence_of_element_located(
                    (By.ID, "ctl00_ContentPlaceHolder1_ddlReg")))
                found_reg = driver.execute_script("""
                    var sel = arguments[0], target = arguments[1].toUpperCase();
                ...
                """, reg_dd, jobcard)
                if found_reg:
                    self.app.log_message(self.log_display, f"     ✅ Registration {jobcard} selected.")
                else:
                    self.app.log_message(self.log_display, f"     ❌ Jobcard {jobcard} not found on DelReg page.")
                    log_one(jobcard, "[Registration]", "Failed",
                            "Jobcard not found on DelReg page.")
                    return
            except Exception as e:
                self.app.log_message(self.log_display, f"     ❌ Registration error: {str(e).splitlines()[0]}")
                log_one(jobcard, "[Registration]", "Failed",
            ...
            # ── Select Reason ──
            try:
                self.app.log_message(self.log_display, f"📝 Selecting reason: {reg_reason}")
            ...
            # ── Submit ──
            self.app.log_message(self.log_display, "📤 Submitting registration deletion...")
            try:
    ...
    def export_report(self):
        if not self.results_tree.get_children():
            messagebox.showinfo("No Data", "No results to export.", parent=self.app)
            return
        ...
        for item in self.results_tree.get_children():
            ...
            status = str(vals[4]).upper()  # Status is column index 4
            if filter_mode == "Export All":
                data_rows.append(vals)
            elif filter_mode == "Success Only" and "SUCCESS" in status:
                data_rows.append(vals)
            elif filter_mode == "Failed Only" and "SUCCESS" not in status:
                data_rows.append(vals)

        if not data_rows:
            messagebox.showinfo("No Data", f"No records match filter '{filter_mode}'.", parent=self.app)
            return
        ...
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"delete_applicant_report_{timestamp}.xlsx",
            title="Save Professional Excel Report")
        if not filename:
            return
        ...
        try:
            ...
            c = ws['A1']
            c.value = "🗑️  Applicant Deletion Report"
            c.font = Font(size=14, bold=True, color="FFFFFF")
            c.fill = hdr_fill
            c.alignment = center

            ws.merge_cells('A2:F2')
            ws['A2'].value = f"Generated by NregaBot.com | {datetime.now().strftime('%d-%m-%Y %I:%M %p')}"
            ws['A2'].font = Font(italic=True, size=9, color="666666")
            ws['A2'].alignment = center

            # Panchayat info
            ws.merge_cells('A3:F3')
            ws['A3'].value = f"Panchayat: {self.excel_panchayat or 'Auto-detected'}"
            ws['A3'].font = Font(size=10, bold=True, color="333333")

            # Summary row
            total = len(data_rows)
            success = sum(1 for r in data_rows if 'success' in str(r[4]).lower())
            failed = total - success
            ws.merge_cells('A4:F4')
            ws['A4'].value = f"Total: {total}  |  ✅ Success: {success}  |  ❌ Failed: {failed}"
            ws['A4'].font = Font(size=10, bold=True, color="006100" if success == total else "CC0000")

            # Table header (row 6)
            headers = ["#", "Deletion Date", "Jobcard No", "Applicant Name", "Status", "Details"]
            col_widths = [6, 16, 22, 30, 12, 45]
            row_num = 6
            for ci, h in enumerate(headers):
                c = ws.cell(row=row_num, column=ci+1, value=h)
                c.font = hdr_font; c.fill = hdr_fill; c.alignment = center; c.border = thin_border
                ws.column_dimensions[chr(65+ci)].width = col_widths[ci]

            # Data rows
            for idx, row in enumerate(data_rows):
                rn = row_num + 1 + idx
                is_success = 'success' in str(row[4]).lower()
                fill = green_fill if is_success else red_fill
                for ci in range(len(headers)):
                    val = row[ci] if ci < len(row) else ""
                    c = ws.cell(row=rn, column=ci+1, value=val)
                    c.border = thin_border
                    if ci in (0, 1, 4):
                        c.alignment = center
                    if ci == 4:  # Status
                        c.font = Font(bold=True, color="006100" if is_success else "CC0000")
                    c.fill = fill

            wb.save(filename)
            messagebox.showinfo("Success", f"Report saved!\n{filename}", parent=self.app)
            try:
