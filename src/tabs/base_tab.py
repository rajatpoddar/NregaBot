...
class BaseAutomationTab(ctk.CTkFrame):
    ...
    def retry_failed_automation(self, input_widget: Any) -> None:
        ...
        # columns) so retry keeps working even when a tab adds a Panchayat
        # column or reorders columns.
        ...
        if columns:
            for i, c in enumerate(columns):
                ...
                if cl == 'status':
                    status_idx = i
                if any(kw in cl for kw in ('work code', 'workcode', 'jobcard', 'job card', 'key', 'item')):
                    code_idx = i

        for item_id in all_items:
            values = self.results_tree.item(item_id)['values']
            code = str(values[code_idx]) if len(values) > code_idx else str(values[0])
            status = str(values[status_idx]).lower() if len(values) > status_idx else str(values[1]).lower()
    ...
    def export_treeview_to_excel(self, tree: Any, default_filename: str = "report.xlsx",
        ...
        try:
            ...
            # ═══════════════════════════════════════════════
            # PRINT SETUP — office-ready (A4 landscape, fit to width)
            # ═══════════════════════════════════════════════
            self._apply_print_setup(ws, data_start_row)
    ...
    @staticmethod
    def _apply_print_setup(ws: Any, header_row: Optional[int] = None) -> None:
        """Office-ready print setup for Excel sheets: A4 landscape,
        fit-to-width, repeated header row on every page and a page-number
        footer — taaki report print karke office me submit ki ja sake."""
        try:
            ...
            ws.oddFooter.right.text = "Generated by NregaBot.com"
        except Exception:
            pass
