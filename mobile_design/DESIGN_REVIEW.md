...
8. **Android app — M6 (devices, activity, support chat, 7 Sep):** **Devices (C7):** list `/api/validate` payload me embedded aata hai (`devices[]` = `device_service.build_device_list` — id, name, is_pending, last_seen, platform, app_version), isliye alag call nahi. Rename `POST /api/set-device-name` **optimistic** (failure par revert); removal `POST /api/request-deactivation` — confirm-first, kabhi optimistic nahi (destructive action wait karta hai), server admin-approval + 30-din per-device rate limit (429). **Activity feed (C8):** `GET /api/activity-log` feed mode, 30/page id-cursor, day-header grouping with per-day count + failure tally; filter chips server-side `automation_key` prefix par map hote hain — All · Muster (`muster`) · Payment (`fto`, desktop ka e-FMS payment step) · Report (`report`). Read-only. **Support chat (C9):** desktop chat tab wali hi WhatsApp Business side se relay; server ka AI assistant thread me `sender="admin"` bankar jawab deta hai, polling `GET /api/whatsapp-chat/messages?since_id` se aata hai. **Persisted outbox (C9b):** har outgoing message send se PEHLE Room `chat_outbox` me likha jata hai — app mar bhi jaye to queued message bacha rehta hai; marks queued (dashed) → sent → poll par asli row se replace. `client_msg_id` per message = idempotent retry (server migration 031: partial unique on license + client_msg_id). **Room v2→v3:** `activity_entry` (per-filter page 1 cached), `chat_message`, `chat_outbox` — cache-first, offline par last loaded feed/thread. Support bottom-nav slot nahi leta — Home/Account se overlay khulta hai.
9. **Android app — M7 (FCM push + inbox + expiry reminders, 7 Sep):** **Token lifecycle** (`data/FcmManager.kt`): login par `POST /api/register-fcm-token` (idempotent — same `device_id` UPSERT), Firebase token rotate hone par re-register (`NregaMessagingService.onNewToken`), logout par sabhi token unregister taaki server signed-out phone ko push na kare. **Push receipt** (`push/NregaMessagingService.kt`): har push me title/body/category/deeplink; inbox row server par pehle se hai, isliye tray notification sirf heads-up hai — record nahi. **Channels (C10c):** 6 channels (default, license, desktop, support, announcement, expiry reminder), sab `IMPORTANCE_LOW` — reminder batata hai, alarm nahi bajata; sound ke liye user system Settings se opt-in kare. App start par `AppContainer` se register. **Inbox (C10):** `GET /api/notifications` (30/page, id cursor, optional category) + `POST /api/notifications/read` (ids ya all); Room `notification` table hi UI ka source hai — screen cache se turant khulti hai, network refresh peeche page replace karta hai (F3: UI kabhi network call observe nahi karta). Entry points: Home header ka bell + Account ka settings row, dono par unread count. **Local expiry reminders (F5)** (`push/ExpiryReminderWorker.kt`): daily `PeriodicWorkRequest` (unique, KEEP, 15-min initial delay, no network constraint) cached licence Room se padhta hai aur 3-din window me system notification post karta hai, deeplink renewal par. Ye FCM-free safety net hai — push unconfigured ya denied ho tab bhi chalta hai; Android 13+ `POST_NOTIFICATIONS` grant respect karta hai (na ho to chup-chaap skip). **Room v3→v4:** `notification` (7-din cache) + `(isRead, createdAt)` index badge query ke liye. **Firebase stub mode:** app jaan-bujh kar `google-services.json` ke BINA build hota hai — SDK stub me init hoti hai, `getToken()` harmless fail, inbox + WorkManager reminders chalte rehte hain; sirf real push inert hai. JSON `app/` me daalte hi active — code change zero.
10. **Android app — repo + build green (8 Sep):** item 3 me `android-app/` ko "new nested repo" likha tha, par asal me koi repo bani hi nahi thi — ~7,200 lines Kotlin bina kisi version control ke pade the. Ab `git init` + initial commit ho gaya (86 files; `local.properties`, `build/`, `.gradle/`, `*.apk` ignored; koi secret/keystore/`google-services.json` track nahi). Desktop repo ki `.gitignore` me `android-app/` add (nrega-server jaisa hi). **Build break fix:** M7 ka aakhri edit kabhi compile hua hi nahi tha — `ExpiryReminderWorker` ek non-existent `AppJson` reference kar raha tha (serializer ka naam `ApiJson` hai) aur `AccountScreen` ke notifications row me `collectAsState` / `Icon` / `Modifier.size` imports missing the (7 errors, 2 files). Fix ke baad `./gradlew :app:assembleDebug` **BUILD SUCCESSFUL** (13 MB APK) aur CI ka locale-parity gate bhi pass — **190 keys, 1 locale in parity**. Version abhi `0.1.0-M6` (versionCode 6) hi hai; M7 bump phone test ke saath jayega. Baaki pending (user): NAS deploy, M5 ka ₹1 test order + coupon + pending/failure states, M4 ka Hindi-locale phone check, M6/M7 phone test, aur real push ke liye `google-services.json`.
11. **Android app — crash-on-open fix (8 Sep):** purane install ke upar update karne par app khulte hi crash kar rahi thi; **fresh install theek chalti thi** — isi wajah se laga ki app "achanak toot gayi". Wajah: `MIGRATION_3_4` ne `index_notification_isRead_createdAt` banaya tha, par `NotificationRow` entity ne wo index declare nahi kiya tha. Room migration ke baad ka schema entity se validate karta hai aur columns ke sath **indices bhi compare** karta hai — expected 0 index, found 1 → `IllegalStateException: Migration didn't properly handle notification(...)`. `AppDatabase.get()` boot path par `AppGraph` se call hota hai, isliye ye throw pehli composition me hi crash bankar aata tha; fresh install par migration chalti hi nahi (`createAllTables` me bhi index nahi tha) isliye validation pass ho jati thi. **Fix:** entity par `indices = [Index(value = ["isRead", "createdAt"])]` — Room ka default index naam wahi hai jo migration pehle se use kar rahi thi, isliye upgrade path match karta hai aur fresh install bhi wahi index banata hai (badge query ko chahiye bhi). Verify: generated schema dono path par index expect karta hai, purana mismatch sqlite par direct reproduce karke gaya hua confirm kiya, `assembleDebug` green. **Aage ke liye rule: `Migration` me joda gaya har index `@Entity` par bhi declare hona chahiye.**
12. **Android app — asli crash-on-open (phone par diagnose, 8 Sep):** item 11 ka Room index fix zaroori to tha par **wo crash nahi tha jo user hit kar raha tha** — app usse ek step pehle hi mar rahi thi, Room ka code chalta hi nahi tha. Logcat (real device, Android 16): `FATAL EXCEPTION: main` → `Cannot create an instance of class AppViewModel` → `Caused by: javax.crypto.AEADBadTagException` at `EncryptedSharedPreferences.create` / `SecureStore.<init>` → `Caused by: android.security.KeyStoreException: Signature/MAC verification failed`. **Wajah:** `EncryptedSharedPreferences` ke do hisse hote hain — prefs file (jisme Tink keyset AUR values dono hain) aur Keystore master key jo us keyset ko unwrap karti hai. Ye do alag ho sakte hain: backup/restore ya device transfer file to le jata hai par Keystore key kabhi nahi, aur OS khud bhi key invalidate kar sakta hai. Mismatch hote hi `create()` har launch par throw karta hai — aur `SecureStore` `AppContainer` ka pehla field hai jo boot path par `AppGraph` se banta hai, isliye ye throw UI banne se pehle `AppViewModel` ke constructor se bahar aata tha = permanent crash loop, nikalne ka ek hi rasta tha app data clear karna. **Fix (a):** `SecureStore` ab khud theek ho jata hai — failure par **dono** hisse delete karta hai (prefs file + `_androidx_security_master_key_`) aur dobara banata hai; sirf ek hissa hatane se agli launch par wahi mismatch reh jata. Ciphertext sach me unreadable hai, isliye stored license key chali jati hai aur user dobara key se login karta hai — dead app se behtar hai. **Fix (b):** secure file ab backup se excluded hai (`backup_rules.xml` Android 11 aur neeche ke liye, `data_extraction_rules.xml` 12+ ke liye, dono manifest me wired) — sabse aam trigger hi khatam. **Verify (real phone, app data mitaye bina purane install ke upar):** koi `FATAL EXCEPTION` nahi, process alive, secure prefs file reset hokar sirf keyset entries par aa gayi, aur Room cache 2 → 4 migrate ho gaya — `activity_entry` / `chat_message` / `chat_outbox` / `notification` tables + `index_notification_isRead_createdAt` sab maujood. Yaani item 11 ka index fix bhi asli hardware par confirm ho gaya. **User ke liye note:** is recovery me phone ka saved license key mit jata hai — app ek baar Login screen dikhayegi, key dobara daalni hogi.
